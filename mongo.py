...
class MongoDB:
    ...
    async def update_document(self, collection_name: str, document_id: str, update_fields: Dict):
        collection = self.db[collection_name]
        await collection.update_one({"_id": ObjectId(document_id)}, {"$set": update_fields})
